...
public class OnlineTime {
	...
	public static class OnlineTimeMapper extends Mapper<Text, Text, Text, Text> {
	...
	public static class OnlineTimeReducer extends Reducer<Text, Text, Text, NullWritable> {
		...
		protected void cleanup(Reducer<Text, Text, Text, NullWritable>.Context context)
			...
			String B2Rate = String.format("%.2f", 1.0*B2/totalplay * 100) + "%";
				...
				outputkey.set("登陆总数：　"+totalplay+"\t"+"凌晨登陆比率："+A1Rate+"\t"+"上午登陆比率："+A2Rate+"\t"+"下午登陆比率："+B1Rate+"\t"+"晚上登陆比率："+B2Rate);
	...
	public static void main(String[] args) throws Exception {
		...
		Job job;
		job = Job.getInstance(conf, "OnlineTime");
		job.setJarByClass(OnlineTime.class);
		...
		Path outputPath = new Path("/nh-game-logs-3.8.1");
